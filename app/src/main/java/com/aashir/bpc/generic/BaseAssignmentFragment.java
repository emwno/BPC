...
public abstract class BaseAssignmentFragment extends Fragment {
    ...
    public void getOnGoingAssignments(String className, boolean fromPin) {
        if (fromPin) {
            getAssignmentsFromPin(className, className + "_ONGOING", true, false);
        } else {
            getAssignmentsFromParse(className, className + "_ONGOING", true, false);
        }
    }

    public void getOverdueAssignments(String className, boolean fromPin) {
        if (fromPin) {
            getAssignmentsFromPin(className, className + "_OVERDUE", true, true);
        } else {
            getAssignmentsFromParse(className, className + "_OVERDUE", true, true);
        }
    }

    public void getCompletedAssignments(String className, boolean fromPin) {
        if (fromPin) {
            getAssignmentsFromPin(className, className + "_COMPLETED", false, false);
        } else {
            getAssignmentsFromParse(className, className + "_COMPLETED", false, false);
        }
    }

    private void getAssignmentsFromPin(final String className, final String pinName, final boolean notComplete, final boolean overdue) {
            ...
            public void done(List<ParseObject> obj, ParseException e) {
                if (e == null) {
                    ...
                    if (Utils.gotInternet(getActivity()))
                        getAssignmentsFromParse(className, pinName, notComplete, overdue);
                } else {
                    if (isAdded())
    ...
    private void getAssignmentsFromParse(final String className, final String pinName, final boolean notComplete, final boolean overdue) {
        if (mAssignments == null) mAssignments = new ArrayList<>();
    ...
    private void setAssignments(List<ParseObject> obj) {
        ...
        for (ParseObject country : obj) {
            ...
            card.setPostTime(mUtils.getRelativeTimeLong(country.getNumber(Key.Assignment.POST_TIME).longValue()));
            card.setDueTime(mUtils.getRelativeTimeLong(country.getNumber(Key.Assignment.DUE_TIME).longValue()));
